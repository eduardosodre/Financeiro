...
public class LancamentoQueryServiceImpl implements LancamentoQueryService {
	...
	public List<Lancamento> consultaPorFiltro(LancamentoFilter filtro, String timeOffset, Pageable pageable) {
		...
		if (filtro.getValorInicio() != null) {
			predicates.add(builder.lessThanOrEqualTo(root.get("valor"), filtro.getValorInicio()));
		}
		if (filtro.getValorFim() != null) {
			predicates.add(builder.lessThanOrEqualTo(root.get("valor"), filtro.getValorFim()));

		}
		if (filtro.getDataInicio() != null) {
			predicates.add(builder.greaterThanOrEqualTo(root.get("data"), filtro.getDataInicio()));
		}
		if (filtro.getDataFim() != null) {
			predicates.add(builder.lessThanOrEqualTo(root.get("data"), filtro.getDataFim()));
		...
		if (filtro.getTipoLancamento() != null) {
			predicates.add(builder.like(root.get("tipoLancamento"), filtro.getTipoLancamento()));
		...
		if (filtro.getObservacao() != null) {
			predicates.add(builder.like(root.get("observacao"), filtro.getObservacao()));
		...
		query.orderBy(builder.asc(root.get("data")));

		return manager.createQuery(query).getResultList();
	}

	@Override
	public List<Lancamento> consultaPorUsuario(Integer usuario, Pageable pageable) {
		var builder = manager.getCriteriaBuilder();
		var query = builder.createQuery(Lancamento.class);
		var root = query.from(Lancamento.class);
		var predicates = new ArrayList<Predicate>();
